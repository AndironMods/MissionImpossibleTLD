Mission Impossible Mod for The Long Dark - A fully customizable quest systems for long term survival and engagement.

# Overview
Mission Impossible adds a lightweight quest system to The Long Dark.
- New Quest are generated for Daily, Weekly, and Monthly Missions
- Collect randomly selected item
- Receive randomly selected reward
- Progress is tracked automatically
- Rewards are granted at 09:00 the next day

# Features
- Daily/Weekly/Monthly Quests cycle based on in‑game time
- Random collect item (from items_to_collect) List
- Random reward item (from items_as_reward) List
- Automatic daily reset at 09:00
- Progress saved in QuestData.json
- Support for 3 different Difficulties
- Fully customizable Quest and Reward Amount in GearLookup.json
- Fully customizable Daily/Weekly/Monthly Quests Types in GearLookup.json
- Customize Number of Quests per Day/Week/Month
- F2 debug shortcut for testing

# Installation
1. Download MelonLoader 0.7.x from the official site and install it into your TLD directory.
2. Download the ModSettings.dll from the offical site and install it in you Mod Folder.
3. Download Mission Impossible Mod Zip File from the Release Folder.
4. Extract the contents into:
   C:\Program Files (x86)Steam\steamappscommon\TheLongDark\Mods
   Your folder should look like this:
   Mods/
   - MissionImpossible.dll
   - GearLookup.json
6. Update Setting in Options>Mod Settings>Mission Impossible to your needs.

# Requirements
- MelonLoader 0.7.x
- The Long Dark v2.55+
- ModSettings v2.2.5


